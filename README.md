...
/var/www/CatalogApp-Udacity/catalogAppUdacity/client_secrets.json
```
...
* after tweak the catalog application, I create another branch called **online** and push it to **
**. so now i have the same working catalog application that works well on both local machine and a server.
...
~ $ git clone --single-branch https://github.com/mkoueslati/catalogAppUdacity --branch online catalogAppUdacity
now the application is located inside  /var/www/CatalogApp-Udacity/catalogAppUdacity
```
...
~ $ cd /var/www/CatalogApp-Udacity/catalogAppUdacity/
...
```sh
~ $ sudo nano /var/www/CatalogApp-Udacity/catalogAppUdacity.wsgi
```
* and we will need to add the following content inside
```sh
import sys
sys.path.insert(0, '/var/www/CatalogApp-Udacity/catalogAppUdacity')
#activate_this is for activate the packages for virtual environment
activate_this = '/var/www/CatalogApp-Udacity/catalogAppUdacity/venv/bin/activate_this.py'
...
```sh
~ $ sudo nano /etc/apache2/sites-available/catalogAppUdacity.conf
```
...
<VirtualHost *:80>
  ...
  WSGIScriptAlias / /var/www/CatalogApp-Udacity/catalogAppUdacity.wsgi
  <Directory /var/www/CatalogApp-Udacity/catalogAppUdacity/>
      Order allow,deny
      Allow from all
  </Directory>
 Alias /static /var/www/CatalogApp-Udacity/catalogAppUdacity/static
  <Directory /var/www/CatalogApp-Udacity/catalogAppUdacity/static/>
      Order allow,deny
      Allow from all
  </Directory>
